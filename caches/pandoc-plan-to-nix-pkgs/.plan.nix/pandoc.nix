    ...
    components = {
      "library" = {
        ...
        modules = ([
          ...
          "Text/Pandoc/BCP47"
          "Text/Pandoc/Class"
          ] ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "8.4") "Prelude") ++ (pkgs.lib).optional (flags.embed_data_files) "Text/Pandoc/Data";
        hsSourceDirs = [
          "src"
      ...
      exes = {
        "pandoc" = {
          ...
          mainPath = ((([
            "pandoc.hs"
            ] ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "8.4") "") ++ (pkgs.lib).optional (flags.static) "") ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "8.2") "") ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "8.4") "";
          };
        "trypandoc" = {
          ...
          modules = (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "8.4") "Prelude";
          hsSourceDirs = [
            "trypandoc"
            ] ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "8.4") "prelude";
          mainPath = ((([ "trypandoc.hs" ] ++ [
            ""
            ]) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "8.4") "") ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "8.2") "") ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "8.4") "";
          };
        };
      tests = {
        "test-pandoc" = {
