  ...
  components = {
    "library" = {
      ...
      modules = ([
        ...
        "Prelude") ++ (pkgs.lib).optional (flags.embed_data_files)
        "Text/CSL/Data/Embedded";
      hsSourceDirs = [ "src" "compat" ]
        ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "8.4")
        "prelude";
    };
    exes = {
      "pandoc-citeproc" = {
        depends = ([
          ...
          (hsPkgs."base-compat" or (buildDepError "base-compat"));
        buildable = true;
        modules = [ "Paths_pandoc_citeproc" ]
          ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "8.4")
          "Prelude";
        hsSourceDirs = [ "." ]
          ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "8.4")
          "prelude";
        mainPath = ((([ "pandoc-citeproc.hs" ]
          ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "8.0")
          "")
          ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "8.4")
      ...
      "test-citeproc" = {
        depends = ([
          ...
          (hsPkgs."base-compat" or (buildDepError "base-compat"));
        build-tools = [
          (hsPkgs.buildPackages.pandoc-citeproc or (pkgs.buildPackages.pandoc-citeproc or (buildToolDepError
            "pandoc-citeproc")))
        ];
        buildable = if flags.test_citeproc then true else false;
        modules = [ "JSON" "Text/CSL/Compat/Pandoc" ]
          ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "8.4")
          "Prelude";
        hsSourceDirs = [ "tests" "compat" ]
          ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "8.4")
          "prelude";
